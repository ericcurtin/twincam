...
int SDLSink::configure(const libcamera::CameraConfiguration& config) {
  ...
  switch (cfg.pixelFormat) {
    ...
    case libcamera::formats::YVU420:
      ...
      break;
    default:
      std::cerr << cfg.pixelFormat.toString()
...
int SDLSink::start() {
  ...
  if (ret) {
    /* Not critical to set, don't return in this case, set for scaling purposes
     */
...
void SDLSink::renderBuffer(FrameBuffer* buffer) {
  ...
  for (unsigned int i = 0; i < buffer->planes().size(); ++i) {
    ...
    SDL_UpdateTexture(sdlTexture_, nullptr, data.data(), pitch_);
  }

  SDL_RenderClear(sdlRenderer_);  // always outside loop these three
