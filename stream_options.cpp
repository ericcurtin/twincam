...
StreamKeyValueParser::StreamKeyValueParser() {
  addOption("role", OptionString,
            ...
            ArgumentRequired);
  addOption("width", OptionInteger, "Width in pixels", ArgumentRequired);
  addOption("height", OptionInteger, "Height in pixels", ArgumentRequired);
  addOption("pixelformat", OptionString, "Pixel format name", ArgumentRequired);
}
...
KeyValueParser::Options StreamKeyValueParser::parse(const char* arguments) {
  ...
  if (options.valid() && options.isSet("role") && !parseRole(&role, options)) {
    std::cerr << "Unknown stream role " << options["role"].toString()
              << std::endl;
    options.invalidate();
  }
...
int StreamKeyValueParser::updateConfiguration(CameraConfiguration* config,
  ...
  if (config->size() != streamParameters.size()) {
    std::cerr << "Number of streams in configuration " << config->size()
              << " does not match number of streams parsed "
              << streamParameters.size() << std::endl;
    return -EINVAL;
  }
